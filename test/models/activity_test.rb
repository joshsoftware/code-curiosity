...
class ActivityTest < ActiveSupport::TestCase
  ...
  def test_score_if_total_no_of_words_less_than_25
    activity = create(:activity, :issue, description: Faker::Lorem.words, user: create(:user))
  ...
  def test_score_should_be_1_if_no_of_words_are_greater_than_25_but_less_than_40
    activity = create(:activity, :issue, description: Faker::Lorem.words(30, true).join(' '), user: create(:user))
    assert_equal activity.auto_score, nil
    activity.calculate_score_and_set
  ...
  def test_score_should_be_2_if_no_of_words_greater_than_40
    activity = create(:activity, :issue, description: Faker::Lorem.words(45, true).join(' '), user: create(:user))
    assert_equal activity.auto_score, nil
    activity.calculate_score_and_set
  ...
  def test_activities_count_of_user_is_zero_before_any_activity_is_created
    activity = build(:activity, :issue, description: Faker::Lorem.words)
  ...
  def test_consider_for_scoring_scope_should_not_retrive_closed_event_action
    opened_activity = create(:activity, :issue, description: Faker::Lorem.words, event_action: 'opened')
    closed_activity = create(:activity, :issue, description: Faker::Lorem.words, event_action: 'closed')
    assert_equal Activity.considered_for_scoring.count, 1
    assert_equal Activity.considered_for_scoring.first, opened_activity
  end

  def test_consider_for_scoring_scope
    opened_issue = create(:activity, :issue, description: Faker::Lorem.words, event_action: :opened)
    edited_issue = create(:activity, :issue, description: Faker::Lorem.words, event_action: :edited)
    closed_issue = create(:activity, :issue, description: Faker::Lorem.words, event_action: :closed)
    reopened_issue = create(:activity, :issue, description: Faker::Lorem.words, event_action: :reopened)
    ...
    assert_equal Activity.considered_for_scoring.count, 3
    assert_includes Activity.considered_for_scoring.to_a, opened_issue
    assert_includes Activity.considered_for_scoring.to_a, reopened_issue
    assert_includes Activity.considered_for_scoring.to_a, created_comment
    refute_includes Activity.considered_for_scoring.to_a, edited_issue
    refute_includes Activity.considered_for_scoring.to_a, closed_issue
    refute_includes Activity.considered_for_scoring.to_a, edited_comment
    refute_includes Activity.considered_for_scoring.to_a, deleted_comment
  end
  ...
  def test_proper_round_is_assigned
    ...
    activity_1 = create :activity, :issue, description: Faker::Lorem.words, commented_on: Date.today, round: nil
    assert_equal activity_1.round, round_1
    activity_2 = create :activity, :issue, description: Faker::Lorem.words, commented_on: Date.today - 1.month, round: nil
