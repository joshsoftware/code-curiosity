...
class OrganizationTest < ActiveSupport::TestCase
  ...
  def test_github_handle_must_be_present
    ...
    organization.valid?
    puts(organization.last_repo_sync_at)
  ...
  def test_repo_syncing_when_last_repo_sync_time_is_less_than_an_hour
    ...
    organization = create(:organization, :github_handle => 'dolores', :last_repo_sync_at => repo_sync_time)
  ...
  def test_organisation_informations_are_not_nil
    organization = build(:organization, :github_handle => 'dolores')
  ...
  def test_organization_setup
    ...
    assert_difference 'Organization.count' do
      Organization.setup('dolores', admin)
    end 
  end

  def test_created_organization_must_have_user
    admin = create(:user)
    org = Organization.setup('dolores', admin)
