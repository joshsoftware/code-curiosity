...
class RedeemRequestTest < ActiveSupport::TestCase
  ...
  test "validity of redeem request" do
  ...
  test "update transaction_points only when points greater than zero" do
    transaction = create(:transaction, :points => 3, :type => 'credit')
  ...
  test "user total_points must be greater than or equal to redeemption_points" do
    user = create(:user)
  ...
  test "points not in mutiple of_hundred so no redeemption" do
  ...
  test "transaction corresponding to redeem_request must be destroyed when it is deleted" do
    user = create(:user)
    transaction = create(:transaction, :type => 'credit', :points => 5, user: user)
