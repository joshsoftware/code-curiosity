...
class SponsorerDetailsControllerTest < ActionController::TestCase
  ...
  test "creates a valid stripe customer with subscription" do
    ...
    assert_difference 'SponsorerDetail.count' do
      post :create, sponsorer_detail: { sponsorer_type: "ORGANIZATION", avatar: file, publish_profile: "1", payment_plan: "basic" }, stripeToken: stripe_helper.generate_card_token(last4: '4242', exp_year: Time.now.year + 1), stripeEmail: @user.email
  ...
  test "does not create sponsorer if error while creating stripe subscription" do
    ...
    assert_difference 'SponsorerDetail.count', 0 do
      post :create, sponsorer_detail: { sponsorer_type: "ORGANIZATION", avatar: file, publish_profile: "1", payment_plan: "basic" }, stripeToken: 'invalid_token1', stripeEmail: @user.email
    end
    assert_not_nil flash[:error]
  end
